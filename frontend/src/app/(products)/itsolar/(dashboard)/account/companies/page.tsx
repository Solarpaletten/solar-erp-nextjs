...
export default function CompaniesPage() {
  ...
  const savePriorities = async (updatedCompanies: Company[]) => {
    try {
      ...
      try {
        await fetch('/api/itsolar/account/companies/priorities', {
          method: 'PUT',
  ...
  const fetchCompanies = async () => {
    try {
      ...
      const response = await fetch('/api/itsolar/account/companies/stats');
  ...
  const handleEnterCompany = async (companyId: number) => {
    try {
      ...
      const response = await fetch('/api/itsolar/account/switch-to-company', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      router.push(`/itsolar/company/${companyId}/dashboard`);
    } catch (error: any) {
  ...
  const handleCreateCompany = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/itsolar/account/companies', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleEditCompany = async (e: React.FormEvent) => {
    ...
    const response = await fetch(`/api/itsolar/account/companies/${editingCompany.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteCompany = async (company: Company) => {
    const response = await fetch(`/api/itsolar/account/companies/${company.id}`, { method: 'DELETE' });
    if (response.ok) {
        ...
        <div className="mt-8 text-center text-sm text-gray-500">
          <div className="bg-white rounded-lg shadow-sm p-4 max-w-2xl mx-auto">
            ...
            <div className="space-y-1 text-left">
              ...
              <p>• Endpoint: <code>/api/itsolar/account/companies</code></p>
